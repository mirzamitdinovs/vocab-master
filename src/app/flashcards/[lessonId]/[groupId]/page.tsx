...
export default function FlashcardLessonGroupPage() {
  return <UserGate>{(user) => <FlashcardLessonGroupView user={user} />}</UserGate>;
}

function FlashcardLessonGroupView({ user }: { user: User }) {
  ...
  const resumeKey =
    lessonId && groupIdParam
      ? `flashcards-session:${lessonId}:group:${groupIdParam}`
      : '';
  ...
  async function handleCompleteSession() {
    ...
    router.push(`/${locale}/flashcards/${lessonId}`);
  }
    ...
    <div className="p-2 h-full flex flex-col gap-4">
      <header className=" border-b rounded-lg bg-white px-4 py-3">
        <div className="flex items-center justify-between gap-4">
          ...
          >
            <Link href={`/${locale}/flashcards/${lessonId}`}>
