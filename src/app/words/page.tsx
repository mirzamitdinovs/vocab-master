...
function WordsView() {
  ...
  const [levelsByLanguage, setLevelsByLanguage] = useState<
    Record<string, Level[]>
  >({});
  ...
  useEffect(() => {
    Object.values(levelsByLanguage)
      .flat()
      .forEach((level) => {
        ...
        return {
          ...
          levels: levels.map((level) => ({
            ...
            chapters: (chaptersByLevel[level.id] ?? [])
              ...
              .map((chapter) => ({
                chapter,
                words: (wordsByChapter[chapter.id] ?? [])
                  .slice()
                  .sort((a, b) => a.order - b.order),
              })),
          })),
    ...
    <div className="space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Browse lessons and words added by admin.
        </p>
      </header>
              ...
              {block.levels.map((level) => (
                <div key={level.level.id} className="mb-6 space-y-3">
                  ...
                  <Accordion
                    ...
                    value={openChapter}
                    onValueChange={(value) =>
                      setOpenChapter(value || undefined)
                    }
                    className="space-y-3"
                  >
